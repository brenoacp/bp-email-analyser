...
export const SegCard: React.FC<SegCardProps> = ({ seg, clientMetadata }) => {
  const getSclDetails = (scl: number | null) => {
    if (scl === null) return { text: 'N/A', color: 'text-slate-400 bg-slate-800 border-slate-700', label: 'Não informado' };
    if (scl === -1) return { text: '-1', color: 'text-sky-300 bg-sky-500/20 border-sky-500/30', label: 'Bypass Interno / Seguro' };
    if (scl >= 0 && scl <= 1) return { text: `${scl}`, color: 'text-emerald-300 bg-emerald-500/20 border-emerald-500/30', label: 'Mensagem Legítima (Não-Spam)' };
    if (scl >= 2 && scl <= 4) return { text: `${scl}`, color: 'text-amber-300 bg-amber-500/20 border-amber-500/30', label: 'Suspeita Leve' };
    if (scl >= 5 && scl <= 6) return { text: `${scl}`, color: 'text-orange-300 bg-orange-500/20 border-orange-500/30', label: 'Spam Provável' };
    return { text: `${scl}`, color: 'text-rose-300 bg-rose-500/20 border-rose-500/30', label: 'Spam de Alta Confiança' };
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {clientMetadata?.dangerous_attachments.map((ext, i) => (
                <span key={i} className="px-2.5 py-1 rounded bg-rose-950/80 border border-rose-600/50 text-xs font-mono font-bold text-rose-200">
                  {ext}
                </span>
              ))}
            </div>
      ...
      {isM365Phishing && (
        <div className="p-4 sm:p-5 rounded-2xl bg-rose-500/10 border-2 border-rose-500/40 text-rose-200 flex items-start gap-4">
          <div className="p-2.5 rounded-xl bg-rose-500/20 text-rose-400 shrink-0">
          ...
          <div>
            ...
            <div className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-300 block">SCL (Spam Confidence Level)</span>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/60">
                <div>
                  <span className="font-semibold text-slate-300 block">BCL (Bulk Complaint Level)</span>
                  <span className="text-[11px] text-slate-500">Índice de envio em massa (0 a 9)</span>
                </div>
                <span className="text-xs font-bold font-mono px-2.5 py-1 rounded-lg bg-slate-800 text-slate-300 border border-slate-700">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/60">
                <div>
                  <span className="font-semibold text-slate-300 block">CAT (Categoria de Ameaça)</span>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/60">
                <div>
                  <span className="font-semibold text-slate-300 block">SFV (Spam Filtering Verdict)</span>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/60">
                <div>
                  <span className="font-semibold text-slate-300 block">Exchange AuthAs</span>
                ...
                {seg.sandbox_detected && seg.sandbox_names.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {seg.sandbox_names.map((name, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded bg-purple-500/10 border border-purple-500/20 text-purple-300 text-xs font-medium"
                      >
                        {name}
                      </span>
                    ))}
                ...
                <div className="pt-3 border-t border-slate-800/60 space-y-2">
                  ...
                  {seg.google_smtp_source && (
                    <div className="p-2 rounded bg-slate-950 border border-slate-800 text-[11px] font-mono text-slate-300">
                      ...
                      <span className="text-sky-400 break-all">{seg.google_smtp_source}</span>
                    </div>
                  )}
                  {seg.google_message_state && (
                    <div className="p-2 rounded bg-slate-950 border border-slate-800 text-[11px] font-mono text-slate-300">
                      <span className="text-slate-500">X-Gm-Message-State: </span>
                      <span className="text-slate-400 break-all">{seg.google_message_state}</span>
                    </div>
                  )}
                </div>
              )}
      ...
      {clientMetadata && (
        <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-md backdrop-blur-sm">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-800 text-slate-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 text-xs">
            ...
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/60">
              ...
              <span className="font-mono text-slate-200 block break-all">
                {clientMetadata.x_mailer || clientMetadata.user_agent || <span className="text-slate-500 italic">Não informado</span>}
              </span>
              {clientMetadata.suspicious_client && (
                <span className="mt-2 inline-flex items-center gap-1 text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-rose-500/20 text-rose-300 border border-rose-500/30">
            ...
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/60">
              ...
              <span className="font-mono text-slate-200 block break-all text-[11px]">
                {clientMetadata.message_id || <span className="text-slate-500 italic">Ausente</span>}
              </span>
              <div className="mt-2 flex items-center gap-1.5">
                ...
                {clientMetadata.message_id_domain && (
                  <span className="text-[11px] font-mono text-slate-400 truncate" title={clientMetadata.message_id_domain}>
                    @{clientMetadata.message_id_domain}
                  </span>
                )}
              </div>
