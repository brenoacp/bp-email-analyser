        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-rose-500/10 border-2 border-rose-500/40 text-rose-900 dark:text-rose-200 flex items-start gap-4 shadow-lg shadow-rose-950/10 dark:shadow-rose-950/30">
          ...
          <div>
            ...
            <p className="text-xs text-rose-800 dark:text-rose-300 mt-1.5 leading-relaxed">
              O endereço IP de origem (<code className="bg-rose-100 dark:bg-rose-950/80 px-1 py-0.5 rounded text-rose-950 dark:text-white font-mono">{originIp.ip}</code>)
            ...
            <div className="flex flex-wrap gap-2 mt-2.5">
              {originIp.rbl_listings.map((rbl, idx) => (
                <span key={idx} className="px-3 py-1 rounded-lg bg-rose-100 dark:bg-rose-950/80 border border-rose-300 dark:border-rose-600/50 text-xs font-mono font-bold text-rose-900 dark:text-rose-200">
                  {rbl}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="mt-4 space-y-3 text-xs">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">{ageBadge.description}</p>
              </div>

              {/* Data de Registro */}
              <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/60">
                <span className="text-slate-500 dark:text-slate-400">Data de Registro:</span>
              ...
              {/* Registrar */}
              <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/60">
                <span className="text-slate-500 dark:text-slate-400">Registrador (Registrar):</span>
                <span className="font-medium text-slate-800 dark:text-slate-200 truncate max-w-[200px]" title={domainInfo.registrar || ''}>
              ...
              {/* País de Registro */}
              <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/60">
                <span className="text-slate-500 dark:text-slate-400">País Declarado:</span>
                ...
                {domainInfo.has_mx && domainInfo.mx_records.length > 0 ? (
                  <div className="space-y-1">
                    {domainInfo.mx_records.slice(0, 3).map((mx, idx) => (
                      <div
                        key={idx}
                        className="p-1.5 rounded bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 font-mono text-[11px] text-slate-700 dark:text-slate-300 truncate"
                        title={mx}
                      >
                        {mx}
                      </div>
                  ...
                  <div className="p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-300 text-[11px]">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-md backdrop-blur-sm">
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              ...
              {originIp.is_private ? (
                <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                  <Lock className="w-3 h-3" /> Rede Privada
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold px-2 py-0.5 rounded bg-sky-500/20 text-sky-700 dark:text-sky-300 border border-sky-500/30">
                  IP Público
                </span>
              )}
            </div>
            ...
            <div className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-slate-400">Endereço IP:</span>
                {originIp.ip ? (
                  <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-950 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-800 font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                    ...
                    >
                      {copiedIp ? <Check className="w-3 h-3 text-emerald-500 dark:text-emerald-400" /> : <Copy className="w-3 h-3" />}
                    </button>
                  </div>
                ) : (
                  <span className="text-slate-400 dark:text-slate-500 italic">Não identificado</span>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800/60">
                ...
                <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 font-mono text-[11px] text-slate-700 dark:text-slate-300">
                  <span className="text-sky-600 dark:text-sky-400 font-semibold">{originIp.asn || 'ASN N/D'}</span>
                  {originIp.org && <span className="text-slate-500 dark:text-slate-400"> — {originIp.org}</span>}
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800/60">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {originIp.rbl_listed ? (
                      <>
                        <ShieldAlert className="w-3 h-3 text-rose-500 dark:text-rose-400" /> Listado
                      </>
                    ) : (
                      <>
                        <ShieldCheck className="w-3 h-3 text-emerald-500 dark:text-emerald-400" /> Limpo
                      </>
                    )}
                  </span>
                </div>
