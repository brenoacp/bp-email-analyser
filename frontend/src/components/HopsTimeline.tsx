...
export const HopsTimeline: React.FC<HopsTimelineProps> = ({ hops }) => {
  ...
  if (!hops || hops.length === 0) {
    return (
      <div className="p-8 rounded-2xl bg-slate-900/80 border border-slate-800 text-center text-slate-500">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
          ...
          <span className="text-lg font-bold text-white mt-0.5 block">{hops.length} MTAs</span>
        </div>
        <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
        ...
        <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
          ...
          <span className={`text-lg font-bold mt-0.5 block ${fcrdnsFailedCount > 0 ? 'text-rose-400' : 'text-slate-400'}`}>
            {fcrdnsFailedCount}
          </span>
        </div>
      </div>
        ...
        {hops.map((hop, idx) => {
          ...
          return (
            <div key={`${hop.order}-${idx}`} className="relative group">
              ...
              <div
                className={`absolute -left-[31px] sm:-left-[39px] top-1.5 w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-xs font-bold ring-4 ring-slate-950 transition-colors ${
                  isFirst
              ...
              {/* Card do Salto */}
              <div className="p-4 sm:p-5 rounded-xl bg-slate-900/90 border border-slate-800 hover:border-slate-700 transition-all shadow-md backdrop-blur-sm">
                {/* Cabeçalho do Card */}
                <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-800/80">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {hop.is_private && (
                      <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20 flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    {hop.delay_seconds !== null && hop.delay_seconds !== undefined && (
                      ...
                      >
                        ...
                        +{hop.delay_seconds}s
                        {isHighLatency && <AlertTriangle className="w-3 h-3 text-rose-400 ml-1" />}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3 text-xs">
                  <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
                    ...
                    <span className="font-mono text-slate-200 break-all">
                      {hop.from_host || <span className="text-slate-600 italic">Não informado</span>}
                    </span>
                  </div>
                  ...
                  <div className="p-2.5 rounded-lg bg-slate-950/60 border border-slate-800/60">
                    ...
                    <span className="font-mono text-slate-200 break-all">
                      {hop.by_host || <span className="text-slate-600 italic">Não informado</span>}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mt-3 pt-3 border-t border-slate-800/60">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-400 font-medium">IP:</span>
                    {hop.ip ? (
                      <div className="flex items-center gap-1.5 bg-slate-950 px-2.5 py-1 rounded-md border border-slate-800 text-xs font-mono text-sky-400">
                        <span>{hop.ip}</span>
                        <button
                          type="button"
                      ...
                      <span className="text-xs text-slate-600 italic">Sem IP extraído</span>
                    )}
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400">
                    ...
                    {(hop.asn || hop.org) && (
                      <span className="flex items-center gap-1 text-slate-400" title={hop.org || hop.asn || ''}>
